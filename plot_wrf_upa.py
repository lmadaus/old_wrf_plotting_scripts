...
# Set your projection. It can be either merc for 'Mercator' or lcc for 'Lambert Conformal Conic'.
projection = 'merc'
...
# The smaller the number the more the wind barbs
thin = 3
...
if projection == 'lcc':
    ...
    west_lon = (cen_lon - (((width_meters / 1000) /110) / 2))
    east_lon = ((((width_meters / 1000) /110) / 2) + cen_lon)
    south_lat = (cen_lat - (((height_meters / 1000) /110) / 2))
    north_lat = ((((height_meters / 1000) /110) / 2) + cen_lat)
    ...
    m = Basemap(projection='merc', llcrnrlat=south_lat, urcrnrlat=north_lat,\
                llcrnrlon=west_lon, urcrnrlon=east_lon, lat_ts=20, resolution='h')
    ...
    # This sets a subgrid for voricity and other dx/dy calculations
    x_1, y_1 = m(nc.variables['XLONG'][0, 1:(y_dim - 1), 1:(x_dim - 1)], \
                   nc.variables['XLAT'][0, 1:(y_dim - 1), 1:(x_dim - 1)])
...
def drawmap(DATA, TITLESTRING, PROD, UNITS):
    ...
    pylab.title('WRF-ARW %s   Valid: %s' % (TITLESTRING, curtimestring), \
